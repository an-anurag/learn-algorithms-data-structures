...
the whole list once (O(n)O(n) time). And that's just to get started.
...
The order of operations is important here! We're careful to copy current_node.next into next before
setting current_node.next to previous_node. Otherwise "stepping forward" at the end could actually mean stepping back to previous_node!
...
Python 2.7
We return previous_node because when we exit the list, current_node is None. Which means that the last node we
visited—previous_node—was the tail of the original list, and thus the head of our reversed list.
...
running each operation on your sample input to see if the final output is what you expect. This is a great strategy
for any coding interview question.
"""